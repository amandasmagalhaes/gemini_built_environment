...
# Load legacy prediction datasets
...
# Load canvas prediction datasets
...
# Create descriptive tables for all legacy models
tables <- purrr::map(
  legacy_predictions,
  build_table
)
...
#| fig-width: 14
#| fig-height: 10
# Create plot data - Legacy
plot_data_legacy <- purrr::imap_dfr(
  tables,
...
# Create plot data - Canvas
plot_data_canvas <- purrr::imap_dfr(
  tables_canvas,
  function(df, model_name) {
    df %>%
      filter(row_type == "variable") %>%
      transmute(
        variable,
        agreement,
        kappa,
