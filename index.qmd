...
The spreadsheet was created to support the qualitative review of street segments containing at least two traffic-control features among crosswalks, speed bumps, traffic lights, stop signs, and pedestrian crossing signs, as identified in the field audit. For each selected segment, all available Gemini 3.1 Flash point-level predictions were retained.

Each row represents a Street View point (`point`) within a segment (`identificador`) and includes the image date (`data_imagens_1`) and the first and last segment coordinates to facilitate manual inspection. The column `audit_n` indicates the number of selected features identified in the audit. For each feature, separate columns report the audit (`*_audit`) and Gemini (`*_gemini`) classifications, where 1 indicates presence and 0 indicates absence. This structure enables direct comparison between human and AI classifications across all points within selected segments.
...
# Keep valid observations and extract point number
df_points <- df_points %>%
  filter(
    status == "ok"
  ) %>%
  rename(
    identificador_raw = id,
    data_imagens_1 = pano_date,
    str_cwalk_1 = crosswalks,
    str_scont1_1 = speed_bumps,
    str_tcont3_1 = traffic_light,
    str_tcont1_1 = stop_sign,
    str_cstop_cat2_1 = crossing_sign
  ) %>%
  mutate(
    point = as.numeric(
      str_extract(
...
# Audit data at segment level
base_principal <- read_dta(
  "virtual_audit/Auditoria Virtual Amostra Total - Belo Horizonte v2.dta"
)
df_audit <- base_principal %>%
  mutate(
    identificador = as.character(identificador)
  )
...
# Select segments with at least two traffic features in the audit
segments_selected <- df_audit %>%
  mutate(
    audit_n = rowSums(
      across(
        all_of(
...
# Keep all Gemini points belonging to selected segments
review_segments <- segments_selected %>%
  left_join(
    df_points,
    by = "identificador"
  ) %>%
